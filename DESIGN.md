...
## 主方案：X11 `_NET_WM_WINDOW_TYPE_DESKTOP`（v3）

> **方案演进**：v1 Clutter.Clone → 失败（DXVK 黑帧）→ v2 Reparent → 失败
> （Mutter stacking 冲突）→ **v3 X11 窗口类型直接指定**（当前方案）

### 核心思路

不在 GNOME Shell 层面操作 actor（Clone/Reparent），而是直接在 X11 层
设置 WE 渲染窗口的 `_NET_WM_WINDOW_TYPE` 为 `_NET_WM_WINDOW_TYPE_DESKTOP`。
Mutter 原生支持此窗口类型，自动将窗口放到桌面最底层，不参与 Alt+Tab/概览。

### 数据流

```
WE (Proton/Xwayland)
    └─ DXVK 渲染 → Xwayland 持有窗口纹理
        └─ Mutter 管理该 Xwayland 窗口
            └─ GNOME Shell 扩展 (window-created 信号)：
                ├─ 检测到 WE 窗口 → 立即调 xprop 设 _NET_WM_WINDOW_TYPE_DESKTOP
                ├─ 渲染窗口 (WE_RENDER): raise 到 DESKTOP 层顶部
                └─ 结构窗口 (Progman/WorkerW-icons): opacity=0 透明隐藏
            └─ launch-we.sh 后台循环 (fallback)：
                ├─ 每 5 秒检查并维护 X11 属性
                └─ 设置 input passthrough (XShape 空输入区域)
```

### 为什么不用 Clutter.Clone

Clutter.Clone 对 DXVK/Vulkan 渲染的 Xwayland 窗口显示**纯黑**。
这不是我们代码的问题——GNOME 自己的工作区缩略图对 WE 窗口也显示黑色。
原因是 DXVK 通过 Vulkan 直接向 Xwayland 提交 buffer，
Clutter.Clone 的 re-paint 机制无法读取该 buffer。

测试过的 Clone 变体（全部失败）：
1. Clone WindowActor → 黑
2. Clone 第一个 child (SurfaceActor) → 黑
3. 按 Hanabi 模式挂到 backgroundActor → 黑
4. 小尺寸诊断 Clone 放在最前层 → 仍然黑

### 为什么不 Reparent

将 WindowActor 从 `window_group` 移到 `_backgroundGroup`：
- Mutter 的 `sync_stack()` 会在每次 restacking 时将 actor 移回
- MetaWindow 的输入路由与 actor 位置无关（鼠标仍发给 WE）
- 导致窗口置顶和输入冲突
...
| 组件        | 语言  | 职责                                              |
|-------------|-------|---------------------------------------------------|
| dwmapi.dll  | C     | 创建假桌面层级，标记渲染窗口 (title="WE_RENDER") |
| gnome-ext   | GJS   | window-created 信号 → 立即设 DESKTOP 类型         |
| launch-we.sh| Bash  | 后备属性维护 + input passthrough                   |

### 已知限制

- 鼠标光标在壁纸区域消失（WE 隐藏光标，input passthrough 未解决此问题）
- 概览(Super)中 WE 壁纸在工作区缩略图内，不替换整体背景
- Video 类壁纸因 Wine MF 不完整会崩溃

---

## 历史方案记录：Clutter.Clone 窗口克隆（v2）
...
- ✅ WE 窗口出现在 `global.get_window_actors()` 中（3 个全屏窗口）
- ✅ WM_CLASS = `steam_app_431960`（所有 WE 窗口共用）
- ❌ Clutter.Clone 对 DXVK 窗口显示纯黑 → 放弃，改用 X11 属性方案
...
1. ✅ **窗口可见性**：WE 出现在 `global.get_window_actors()` 中（3 个全屏 + UI 窗口）
2. ✅ **窗口标识**：WM_CLASS = `steam_app_431960`，渲染窗口标记 title="WE_RENDER"
3. ❌ **Clone 正确性**：Clutter.Clone 对 DXVK/Vulkan 窗口显示纯黑（已确认，放弃此方案）
4. ⬜ **遮挡行为**：DESKTOP 类型窗口始终在底层，无遮挡问题
5. ✅ **音频**：WE 壁纸音效通过 PipeWire 正常输出，无需额外处理
...
## Phase 1：GNOME 桌面层集成（X11 窗口类型方案）

> Clutter.Clone 对 DXVK 窗口失败后，改用 X11 `_NET_WM_WINDOW_TYPE_DESKTOP` 方案。

### 方案演进过程

```
尝试 1: Clutter.Clone → 纯黑（DXVK buffer 不可 clone）
尝试 2: Reparent WindowActor → Mutter sync_stack 冲突 + 输入路由不变
尝试 3: _NET_WM_WINDOW_TYPE_DESKTOP → ✅ 成功
```
...
1. **dwmapi-override.c**（Wine 层）：
   - 创建假桌面层级（Progman + 2x WorkerW + SHELLDLL_DefView）
   - WorkerW-render 标题设为 `"WE_RENDER"` 作为 X11 层识别标记
   - WE 查找用 `FindWindowExW(..., NULL)` 不看标题，不影响功能

2. **gnome-ext/extension.js**（GNOME Shell 层）：
   - 监听 `window-created` 信号，WE 窗口出现即刻设置 `_NET_WM_WINDOW_TYPE_DESKTOP`
   - 处理 WM_CLASS 延迟到达的情况（`notify::wm-class` 信号）
   - 渲染窗口 `windowraise`，结构窗口 `opacity=0`

3. **launch-we.sh**（Linux 原生层）：
   - 后台每 5 秒检查并维护 X11 属性（防 Wine 重置）
   - Python3 ctypes 调 `XShapeCombineRectangles(ShapeInput)` 设输入穿透
   - 初始阶段每秒轮询快速响应
...
| 壁纸自动沉底 | ✅ 在所有窗口后面 |
| Alt+Tab 隐藏 | ✅ 不出现 |
| 概览(Super) | ⚠️ 在工作区缩略图内，不替换整体背景 |
| 零延迟启动 | ✅ GNOME 扩展 window-created 信号即时响应 |
| 结构窗口隐藏 | ✅ opacity=0 透明化 |
| 鼠标光标 | ❌ 壁纸区域光标消失（WE 隐藏光标，暂不修复） |

### 源码位置

- `fake-workerw/dwmapi-override.c` — 自定义 dwmapi.dll（含 WE_RENDER 标记）
- `gnome-ext/extension.js` — GNOME Shell 扩展（零延迟 DESKTOP 类型设置）
- `gnome-ext/metadata.json` — 扩展元数据（GNOME 49）
- `launch-we.sh` — Steam 启动脚本（后备属性维护 + input passthrough）
...
| Clutter.Clone 可行性 | **❌ 失败** — DXVK/Vulkan 窗口 Clone 显示纯黑 |
| 遮挡/最小化行为 | N/A（改用 DESKTOP 窗口类型，始终在底层） |
| `get_window_actors()` | ✅ 3 个全屏窗口 + UI 窗口均可见 |
| DXVK Present Mode | `VK_PRESENT_MODE_IMMEDIATE_KHR (dynamic: yes)` |

#### Clutter.Clone 失败详情

测试了 4 种 Clone 变体，全部显示黑色：
1. `Clutter.Clone({ source: windowActor })` → 黑
2. Clone `SurfaceActor`（第一个 child）+ `actor.hide()` → 黑
3. 按 Hanabi 模式添加到 `backgroundActor` 内部 → 黑
4. 480x270 诊断 Clone 放在 stage 最前层 → 仍然黑

GNOME 自身的工作区缩略图对 WE 窗口也显示黑色，确认是
DXVK Vulkan buffer 与 Clutter Clone re-paint 机制的根本性不兼容。

#### 最终方案：X11 窗口类型

放弃 Clutter.Clone/Reparent，改用 `_NET_WM_WINDOW_TYPE_DESKTOP`：
- GNOME Shell 扩展通过 `window-created` 信号零延迟设置
- launch-we.sh 后台循环维护属性 + input passthrough
- dwmapi.dll 标记渲染窗口 title="WE_RENDER" 用于识别
- 结构窗口（Progman/WorkerW-icons）设 opacity=0 透明隐藏
